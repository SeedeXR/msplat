...
void saveGaussianPly(const std::string &path, GaussianParams &p, int step) {
    ...
    // Step prefix shared with the reader via ply_header.hpp (guards the resume bug).
    o << msplat::plyStepCommentPrefix() << step << "\n";
    o << "element vertex " << N << "\n";
    o << "property float x\nproperty float y\nproperty float z\n";
    o << "property float nx\nproperty float ny\nproperty float nz\n";
...
LoadedGaussians loadGaussianPly(const std::string &path, float scale, const float translation[3], bool keepCrs) {
    ...
    // Parse header via the shared, unit-tested parser (keeps the step-comment
    // prefix in sync with the writer — see ply_header.hpp).
    msplat::PlyHeaderInfo hdr = msplat::parsePlyHeader(f);
    if (!hdr.isPly) throw std::runtime_error("Not a PLY file: " + path);
    if (hdr.numPoints == 0) throw std::runtime_error("PLY has no vertices");
    int numPoints = (int)hdr.numPoints;
    int numDc = hdr.numDc;
    int numFr = hdr.numFr;
    int step = hdr.step;
    int frBases = numFr / 3;

    // Robustness: validate the header vertex count against the actual file size
    // before allocating, so a corrupt/hostile header can't trigger a huge alloc.
    {
        std::streampos payloadStart = f.tellg();
        f.seekg(0, std::ios::end);
        std::streamoff remaining = f.tellg() - payloadStart;
        f.seekg(payloadStart);
        long long bytesPerVertex = (long long)(3 + 3 + numDc + numFr + 1 + 3 + 4) * sizeof(float);
        long long needed = (long long)numPoints * bytesPerVertex;
        if (remaining < needed)
            throw std::runtime_error("PLY truncated or header count wrong: need " +
                std::to_string(needed) + " payload bytes, have " + std::to_string((long long)remaining));
    }
